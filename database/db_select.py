...
class SelectQuery(MainDB):
    ...
    async def select_addr_monitoring(self, owner_addr: str):
        """Вытаскиваем данные для таблицы sales_monitoring в Users DB.
        (данные берутся за последний час, чтобы не перегружать выдачу)"""
        async with self.connector as conn:
...
if __name__ == "__main__":
    owner_addr = 'stars16apa8cdhrz5dpnru6khtp2ptvhxxp545l4v5jg'
    s = asyncio.run(SelectQuery().select_addr_monitoring(owner_addr))
    for r in s:
        print(r)
    pass
